 ...
 */
public class code_40_GetLeastNumbers {
    /**
     * 获取数组中最小的k个数
     *
     * @param input 输入的数组
     ...
     *
     * 假设题目要求从海量的数据中找出最小的 k 个数，由于内存的大小是有限的，有可能不能把这些海量的数据一
     * 次性全部载入内存。这个时候，用这种方法是最合适的。就是说它适合 n 很大并且 k 较小的问题。
     */

    public static ArrayList<Integer> GetGetLeastNumbers_2 (int[] input, int k ){
        ArrayList<Integer> res = new ArrayList<>();
        if (input == null || input.length == 0 || input.length < k || k < 1){
            return res;
        }
